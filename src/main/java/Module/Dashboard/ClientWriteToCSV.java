 ...
 * @author yemin
 */
public class ClientWriteToCSV extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Project> projectsList = (ArrayList<Project>) request.getSession().getAttribute("companyProjectList");
        String year = (String) request.getSession().getAttribute("yearClient");
        String name = (String) request.getSession().getAttribute("clientIDSelected");
        ...
        String FILE_HEADER = "completionDate,companyName,projectName,hoursAssigned,hoursActual,difference(%),sales,cost,profit/loss,staff";
        ...
        try {
            ...
            for (Project p : projectsList) {
                ...
                fileWriter.append(sdf.format(p.getDateCompleted()));
                ...
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(df.format(((p.getEmployee1Hours() + p.getEmployee2Hours() - p.getPlannedHours()) / (p.getEmployee1Hours() + p.getEmployee2Hours())) * 100.00));
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(df.format(sales));
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(df.format(cost));
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(df.format(profit));
                fileWriter.append(COMMA_DELIMITER);
                fileWriter.append(employees);
                fileWriter.append(NEW_LINE_SEPARATOR);
            }
        } catch (Exception e) {
